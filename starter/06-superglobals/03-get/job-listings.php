...
// Valid locations (Security measure)
$validLocations = ['New York', 'San Francisco', 'Chicago', 'Seattle', 'Boston'];

function formatSalary($salary)
{
  return '$' . number_format($salary, 2);// number_format() function formats a number with grouped thousands and decimal points.
}

function highlightTags($tags, $searchTerm)
{
  $tagsArray = implode(', ', $tags); // 配列を文字列に変換
...
{
  // use ($location)は関数スコープ内から外の変数にアクセスするため
  return array_filter($listings, function ($koro) use ($location) { // filter_array() function filters elements of an array using a callback function
    return strcasecmp($koro['location'], $location ) === 0; // strcasecmp() function makes a case-insensitive manner () 0: match, 1: not match, -1: error
  });
}

// Check if location query string is there
if (isset($_GET['location']) && in_array($_GET['location'], $validLocations) ) { // in_array() function checks if a value exists in an array
  $location = $_GET['location'];

  // Give entire list and location in the URL
  $filteredListings = filterListingsByLocation($listings, $location);
} else {
  $filteredListings = $listings; // original one
}
