...
{
  return '$' . number_format($salary); // number_format() function formats a number with grouped thousands
}


function highlightTags($tags, $searchTerm)
{
  $tagsArray = implode(', ', $tags); // implode() function joins array elements with a string
...
function calculateAverageSalary($jobListings) {
  $totalSalary = 0;
  $count = count($jobListings); // count() function returns the number of elements in an array

  foreach ($jobListings as $job) {
    $totalSalary += $job['salary']; // salary の合計を計算

    // calculate the average salary
    $averageSalary = ($count > 0 ) ? $totalSalary / $count : 0;

    return formatSalary($averageSalary); // $1,000.00 のようにフォーマットされた平均給与を返す
  }
}
...
function calculateAverageSalary2($jobListings) {
  // array_column() function returns the values from a single column in an array
  $salaries = array_column($jobListings, 'salary'); // take all salaries from the job listings array 

  var_dump($salaries);// [80000, 60000, 55000, 70000, 40000]

  $totalSalary = array_sum($salaries); // array_sum() function returns the sum of all the values in an array
